...
void LCD5110::clearLine(byte _cursorY){

  int starti = _cursorY * LCD5110::WIDTH;
  for (int i = starti; i < starti + LCD5110::WIDTH; i++)
    sendValue(LCD5110::DATA, 0x00);
 ...
 */
void LCD5110::init(byte _sclk_pin, byte _sdin_pin, byte _dc_pin, byte _reset_pin, byte _sce_pin, byte _led_pin){
 ...
 */
void LCD5110::setBacklight(int b){
...
void LCD5110::setContrast(byte contrast)
{
  sendValue(LCD5110::COMMAND, 0x21); //Tell LCD that extended commands follow
  sendValue(LCD5110::COMMAND, 0x80 | contrast); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
  sendValue(LCD5110::COMMAND, 0x20); //Set display mode
}
